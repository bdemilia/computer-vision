...
import labels
import util
from datasets import ImageData, TestData, mlb
...
def get_loader(df, transformations):
    dset_val = ImageData(df, paths.train_jpg, transformations, divide=False)
...
def get_test_loader(test_images, transformations):
    dset_test = TestData(test_images, paths.test_jpg, '.jpg', transformations, divide=False)
